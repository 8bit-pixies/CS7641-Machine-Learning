...
ggplot(final_all[final_all$Description=="results",], aes(x=Run, y=Diff, group=Algorithm)) +
  geom_line(aes(colour=Algorithm)) +
  scale_y_continuous(trans=symlog_trans()) + 
  ylab("Ratio to RHC - Log 10 Scale") +
  xlab("Size of Vector") +
  ggtitle("Ratio of Max Values to RHC") +
  theme_bw() +
  theme(legend.position="bottom")
...
ggsave("fourpeaks.png", p4, width=10, height=4)
