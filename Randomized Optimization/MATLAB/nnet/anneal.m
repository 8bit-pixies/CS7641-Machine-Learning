function [minimum,fval,Trecord,energyrecord,Bestrecord,error_train,error_test] = anneal(loss, parent, options, net, inputs, targets, input_tests, target_tests)
...
elseif nargin<3, %user gave no options structure, use default
    ...
    fs = {'CoolSched','Generator','InitTemp','MaxConsRej',...
        ...
        'Timer', 'Reanneal', 'ReannealSched', 'TolX'};
    for nm=1:length(fs)
...
timer = options.Timer;
heat = options.ReannealSched;
tolX = options.TolX;
k = 1;                           % boltzmann constant

if options.Reanneal < 0;
    reanneal = max_success + 1;
else
    reanneal = options.Reanneal;
end    
...
Bestrecord = oldenergy;
best = oldenergy;
Trecord = T;
energyrecord = oldenergy;
error_train = error_hill(parent', net, inputs, targets);
error_test = error_hill(parent', net, input_tests, target_tests);
...
while ~finished;
    ...
    if itry >= max_try || success >= max_success || msuccess >= max_success;
        if T < minT || consec >= max_consec_rejections;
            ...
            if best > oldenergy;                
                if best - oldenergy < tolX;
                    finished = 1;
                end
                best = oldenergy;
            end
            
            Bestrecord = [Bestrecord best];
            energyrecord = [energyrecord oldenergy];
            error_train = [error_train error_hill(parent', net, inputs, targets)];
            error_test = [error_test error_hill(parent', net, input_tests, target_tests)];
